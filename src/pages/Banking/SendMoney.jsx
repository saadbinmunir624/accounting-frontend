...
const SendMoney = () => {
  ...
  const handleItemChange = (index, itemId) => {
    ...
    if (selectedItem) {
      ...
      // For Send Money (cash out), mirror purchase-side defaults
      nextLine.unitPrice = selectedItem.costPrice || existing.unitPrice || 0;

      const purchaseAccount =
        selectedItem.purchaseAccount && selectedItem.purchaseAccount._id
          ? selectedItem.purchaseAccount
          : selectedItem.purchaseAccount || existing.account;
      nextLine.account = purchaseAccount || null;

      if (taxMode !== 'No Tax') {
        const purchaseTaxId =
          (selectedItem.taxRateOnPurchase && selectedItem.taxRateOnPurchase._id) ||
          selectedItem.taxRateOnPurchase ||
          null;

        if (purchaseTaxId) {
          const safeTaxTypes = Array.isArray(taxTypes) ? taxTypes : [];
          const tax = safeTaxTypes.find((t) => t._id === purchaseTaxId) || null;
  ...
  if (loading) {
    return (
      <div className="p-6">
        <div className="text-sm text-gray-600">Loading send money...</div>
      </div>
    );
  }
